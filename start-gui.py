...
def export_file():
    ...
    with open(save_filename, "w") as cf:
        for entry in entries:
            cf.write(str(entry.entry_label) + " " + str(entry.get_value()
...
import_button = tk.Button(mw, text="Import Design",
                          width=15, command=import_file)
import_button.grid(row=0, column=0, sticky="w")

export_button = tk.Button(mw, text="Export Design",
                          width=15, command=export_file)
...
analyze_button = tk.Button(mw, text="PERFORM ANALYSIS", width=25, height=1, font=(
    "Arial", 17), command=start_analysis, bg="red", fg="white")
...
create_entry("Chamber Stagnation Temperature (T_c)",
             temperature_units, "float")
create_entry("Characteristic Velocity (C*)", velocity_units, "float")
create_entry("Gas Conductivity of Combustion Mixture",
             conductivity_units, "float")
create_entry("Avg. Molecular Mass of Combustion Mix.",
             molecular_mass_units, "float")
